...
namespace ego_planner
{
  ...
  void EGOReplanFSM::init(ros::NodeHandle &nh)
  {
    ...
    if (target_type_ == TARGET_TYPE::MANUAL_TARGET)
    {
      ...
      myWaypoint_sub = nh.subscribe("/move_goal", 1, &EGOReplanFSM::myWaypointCallback, this);
    }
  ...
  void EGOReplanFSM::myWaypointCallback(const geometry_msgs::PoseStampedPtr &msg)
  {
    cout << "My Triggered!" << endl;
    init_pt_ = odom_pos_;
    Eigen::Vector3d end_wp(msg->pose.position.x, msg->pose.position.y, msg->pose.position.z);
    planNextWaypoint(end_wp);
    ros::param::set("EGO_Arrived", false);
  }
